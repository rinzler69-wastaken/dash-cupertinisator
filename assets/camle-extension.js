...
class AbstractCommonMagicLampEffect extends Clutter.DeformEffect {
    ...
    vfunc_set_actor(actor) {
        ...
        if (this.icon.y + this.icon.height >= this.monitor.height - this.EPSILON) {
            ...
            // Save the real dock bottom Y before toTheBorder overwrites icon.y
    ...
    this.iconOriginalX = this.icon.x; // save before toTheBorder
    if (this.toTheBorder) {
    ...
    this.iconOriginalX = this.icon.x + this.icon.width; // save before toTheBorder
    if (this.toTheBorder) {
...
if (this.iconPosition === St.Side.BOTTOM) {
    let clipY = (this.iconOriginalY - this.window.y) * propY - 8;
    if (v.y > clipY) v.y = clipY;
} else if (this.iconPosition === St.Side.LEFT) {
    let clipX = (this.iconOriginalX - this.window.x) * propX + 8;
